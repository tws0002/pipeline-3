...
class ProjectLauncher(navigator.Navigator):
    ...
    def __init__(self, active_window, current_software_tools):
        ...
        self.current_software_tools.debug_msg("Starting project launcher...")
    ...
    def on_launch_click(self):
        """Called when the launch button is clicked. Attempts to launch the project with 
        child's 'launchProject' function
        """
        ...
        if not os.path.isfile(self.finalPath):
            self.current_software_tools.debug_msg("This file doesn't exist! Creating it here: " 
                + self.finalPath)
